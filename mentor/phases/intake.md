...
5. **Draft PLAN.md** per `references/plan-format.md`: mission, milestone list. Milestones are real deliverables in the learner's repo, ordered primitive-first (raw mechanism before the abstraction that hides it). Each milestone names its concepts and its deliverable.
6. **Learner approves the plan.** Present it, invite pushback, revise. No milestone starts until they approve.
7. **Journal** the session per `references/journal-format.md`.
